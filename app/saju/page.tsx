  ...
  return (
    <section className="space-y-4">
      ...
      <form onSubmit={submit} className="grid gap-3 rounded border bg-white p-4 md:grid-cols-2">
        ...
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.unknownBirthTime} onChange={(e) => {
          update("unknownBirthTime", e.target.checked);
          if (e.target.checked) {
            update("birthTime", "");
          }
        }} />출생시간 모름</label>
